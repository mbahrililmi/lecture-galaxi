 ...
 */
package Class;
//merupakan logic untuk data sql mstr_customer/
...
public class mstr_product {
    ...
    public void simpanORupdate() {
        try {
            ...
            _Cnn = DBO.getConnection();
            if (_Akses.equals("")) {
                _sql = "INSERT INTO `store_akhtar`.`mstr_product` (`prd_id`, `nama`, `satuan`, `harga`) VALUES ('?', '?', '?', '?')";
                System.out.println(_sql);
            } else {
                _sql = "UPDATE `store_akhtar`.`mstr_product` (`prd_id` = '" + prd_id + "', `nama` = '" + nama + "', `satuan` = '" + satuan + "', `harga` = '" + harga + "' WHERE `mstr_product`.`prd_id` = '" + prd_id + "'";
                System.out.println(_sql);
            }
            PreparedStatement prs = _Cnn.prepareStatement(_sql);
            prs.setString(1, this.prd_id);
